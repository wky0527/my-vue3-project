...
export default {
  setup() {
    ...
    const backText = (callback) => {
      // if (window.WebSocket) {
      //   let ws = new WebSocket("ws://127.0.0.1:8001")
      //   ws.onopen = function (e) {
      //     console.log('链接服务器成功')
      //     console.log('contentText is', list.contentText)
      //     ws.send(list.contentText)
      //     callback()
      //   }
      ...
      //     list.data = [...list.data, {type: 'robot', content: e.data}]
      //   }
      // }
      uni.connectSocket({
      ...
      // const socket = io('10.1.129.234:8001',{transports:['websocket']})
      // socket.on('connect',function (){
      //   console.log('连接成功')
      // })
      // socket.emit('sendToServer','can you hear me')
      // socket.on('sendToClient',message=>{
      //   list.data = [...list.data, {type: 'robot', content: message.message}]
      // })
